...
| 8.1 framing | Valid magic/footer, bounded metadata, internal column chunk offsets, no external chunk files, row group/file/decoded row agreement, full Arrow array validation. Pin the complete observed source schema across files/days and resumes. **With `--prod-schema-dir` the pin is the declared target schema instead**: files/days may then differ from each other, but each must project onto that one target, and the distinct source schemas seen are recorded in `SOURCE-SCHEMAS.json`. Files within a single day must still agree with each other either way. **With `--accept-clickhouse-74988`**, a column annotated `Timestamp(NANOS)` + converted `UTF8` -- the exact defect of ClickHouse issue 74988 -- has that legacy annotation cleared before the schema is built, as the fixed writer emits; nothing else is altered and the columns are listed in `footer_corrections`. |
...
| 8.7 no value repair | No stripping, coercion, row dropping or deduplication. Only explicit column drops; original typed values are used to regenerate output. **`--prod-schema-dir` is the one exception and the only place this tool writes a value it did not read**: a column a partition predates is written entirely null so the table publishes as one shape. Nothing else is synthesized -- no value present in the source is altered, and no row is added or removed. Every such column is named in that partition's `MANIFEST.json` `padded_columns` and flagged `padded` in `FIELD-AUDIT.json`, because a padded null is not a source null. Padding is refused for any column carrying a semantic type, a field rule or a reviewed per-table exception, since a padded column is not validated. |
